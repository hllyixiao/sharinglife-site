...
public class LoginRegisterController {
    ...
    public boolean register(HttpServletResponse response,
        ...
        if(registerData.nonNull()){
            final User user = new User(registerData);
    ...
    public CommonResponse login(HttpServletResponse response, HttpServletRequest request,
                                HttpSession session, @RequestBody LoginData loginData) {
        LOG.info("login — 用户登陆");
        final CommonResponse commonResponse = new CommonResponse();
        if(loginData.nonNull()){
            if(loginData.getVerifyCode().equalsIgnoreCase(
                    (String) session.getAttribute(Const.VERIFY_CODE))){
                final User user = userService.getUserByLoginData(loginData);
                if(user == null){
                    ...
                    LOG.info("用户-[{}] 登陆成功！", loginData.getPhoneOrName());
                }
            }else{
                commonResponse.setStateCode(0);
